        ...
        表格中：
            按列说明
                ...
                8. 50英尺到接地时间：
                    ...
                    8.1 对于CFM机型：
                        打开CFM飞机的数据表格，参考D列，找到数据第一次从2000以上下降至第一次变为最接近50时的行数并记录为X1行（此时也就是飞机50ft时刻），读取X1行和X行的 `line_index` 字段，计算两个数值的差值绝对值，结果以秒写入，这个值就是50英尺到接地时间

                    8.2 对于LEAP机型：
                        打开LEAP飞机的数据表格，参考D列，找到数据第一次从2000以上下降至第一次变为最接近50时的行数并记录为Y1行（此时也就是飞机50ft时刻），读取Y1行和Y行的 `line_index` 字段，计算两个数值的差值绝对值，结果以秒写入，这个值就是50英尺到接地时间

                    8.3 对于PW机型：
                        打开PW飞机的数据表格，参考D列，找到数据第一次从2000以上下降至第一次变为最接近50时的行数并记录为Z1行（此时也就是飞机50ft时刻），读取Z1行和Z行的 `line_index` 字段，计算两个数值的差值绝对值，结果以秒写入，这个值就是50英尺到接地时间
                ...
                13 起飞离地姿态
                    ...
                    13.1 对于 CFM 机型：
                        ...
                        打开 CFM 飞机的数据表格，自上而下扫描数据行，参考 D 列至 J 列以及 K 列至 Q 列：
                            当同一行中，D-J 列至少有 2 个值大于2，且 K-Q 列至少有 2 个值大于2时，将首次满足该条件的行记录为起飞离地时刻所在行，为X5行。
                    ...
                    13.2  对于 LEAP 机型：
                        ...
                        打开 LEAP 飞机的数据表格，自上而下扫描数据行，参考 D 列至 H 列以及 I 列至 M 列：
                            当同一行中，D-H 列至少有 1 个值大于2，且 I-M 列至少有 1 个值大于2时，将首次满足该条件的行记录为起飞离地时刻所在行，为Y5行。
                            这里记录的是满足条件时所在的行号，不需要区间行数
                        此时按照触发离地条件的高度采样序号，读取Y5行对应的 N列 字段数值，记录为起飞离地姿态
                    13.3 对于 PW 机型：

                        打开 PW 飞机的数据表格，自上而下扫描数据行，参考 D 列至 H 列以及 I 列至 M 列：
                            当同一行中，D-H 列至少有 1 个值大于2，且 I-M 列至少有 2 个值大于2，将首次满足该条件的行记录为起飞离地时刻所在行，为Z5行。
                            这里记录的是满足条件时所在的行号，不需要区间行数
                        此时按照触发离地条件的高度采样序号，读取Z5行对应的 N列字段数值，记录为起飞离地姿态


    2026.8.10 新增功能：
        在生成的qar.xlsx表格中对机型一列进行填充，具体规则如下：
            在最开始选择数据文件夹时将其改为在一个ui界面进行选择数据文件夹以及后续会用到的机型文件：
                机型文件为一个xlsx表格，第一例是飞机注册号，第二列是机型
            在数据分析功能模块中需要分人进行读取文件，读取到的文件名类似于“B-324H_20260706034516___S1_E1_YZDLEAP_CA4184.csv”或者其他类似的名字，需要你先读取其中的“B-324H”及其他文件名中相似的字段，这个即是飞机注册号
            在qar.xlsx文件生成时，需要针对不同的文件，如读取到的是“B-324H_20260706034516___S1_E1_YZDLEAP_CA4184.csv”这个文件，提取飞机注册号为“B-324H”，然后在上述选取的机型文件中寻找与该注册号对应的机型，在此例中机型为“A320LEAP”，然后将得到的机型，即A320LEAP填充至qar.xlsx的对应机型一栏，如果没有找到相应飞机注册号，则在qar.xlsx文件的机型一栏中保持空白
